...
public class EncounterMenu : MonoBehaviour
{
    ...
    public void GoToTargetMenu(Command command){
        ...
        targetParent.AddComponent<RectTransform>();
        targetParent.transform.SetParent(submenus[currMenu].transform, false);
        int count = 0;
        foreach (Unit target in targetSubmenu){
            ...
            targetButton.transform.SetParent(targetParent.transform, false);
            count++;
        }

        //set the scroller content to the parent
        submenus[currMenu].GetComponent<ScrollRect>().content = targetParent.GetComponent<RectTransform>();
    } 
